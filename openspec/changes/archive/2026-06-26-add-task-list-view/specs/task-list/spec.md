...
Vista de grilla de tareas del dashboard, conectada al backend como fuente de verdad. Cubre el composable `useTaskList`, el render adaptativo del `TaskCard` por status, la ocultación de la sesión activa (que vive en `ActiveTimerSection`), y la estrategia de refresh on-demand sin polling. La card CTA "+ AÑADIR TAREA RÁPIDA" preserva el patrón Stitch como entrada a `/tasks/new`.
